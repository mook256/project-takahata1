                ...
                <div class="flex items-center space-x-2">
                    ...
                    <div class="relative">
                        ...
                        <div v-if="dropdownOpen" class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg">
                            <router-link to="">
                                <a @click="gotRequest(token.role)"
                                    class="block px-4 py-2 text-gray-800 hover:bg-gray-200">Request</a>
                            </router-link>
                            <router-link to="">
                    ...
                    <div class="mt-10">
                        ...
                        <div class="flex items-center justify-center space-x-2 flex-grow">
                            <button @click="showModal = true"
                                class="rounded-full bg-green-500 text-white px-4 py-2">Submit</button>
                            <button @click="gotoHome(token.role)"
                                class="rounded-full bg-red-600 text-white px-4 py-2">Cancel</button>
                        </div>
...
const validateForm = () => {
    ...
    const arr = [];
    arr[29] = selectedTopic.value;
    arr[30] = selectedIssue.value;
    arr[31] = detail.value;
    saveData();
    gotoHome(token.role)
};
...
function gotoServices(checkRole) {
    if (checkRole == "0") {
        navigateTo('/home_admin/');
    } else if (checkRole == "1") {
