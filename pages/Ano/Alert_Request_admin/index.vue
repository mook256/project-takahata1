<template>
  <div>
    ...
    <div class="search-bar p-4">
      <input type="text" v-model="searchQuery" placeholder="Search..." class="search-input px-4 py-2 border rounded" />
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="(item, index) in filteredItems" :key="index">
            ...
            <td class="detail-column">
              <button class="underline decoration-red-600 " @click="showFullDetail(item)">Show Detail</button>
            </td>
            <td class="level-column">{{ item.level }}</td>
            <td class="request-approval-column">
              <div class="status-cell">
                <span v-if="item.editingApproval !== index" :style="{ color: getManagerColor(item.request_approval) }"
                  class="status-text">
            ...
            <td class="status-column">
              <div class="status-cell">
                <span v-if="item.editingStatus !== index" :style="{ color: getStatusColor(item.status) }"
                  class="status-text">
                  {{ item.status }}
                </span>
                ...
                <button v-if="item.editingStatus !== index" @click="editStatus(index)" class="edit-button"
            ...
            <td class="detail-admin-column" style="vertical-align: middle;">
              <div style="display: flex; justify-content: space-between; align-items: center; text-align: center;">
                <span>
                  {{ item.detail_admin.length > 10 ? item.detail_admin.substring(0, 10) + '...' : item.detail_admin }}
                </span>
                <div style="display: flex; flex-direction: row; align-items: center;">
                  <button v-if="item.detail_admin.length > 10" @click="showFullAdminDetail(item)" class="more-button"
...
const testClick = async () => {
  try {
    const response = await fetch('http://localhost:8080/api/IT_Request_TableController', {
      method: 'POST',
      credentials: 'include'
    });
...
const Show_On_Table = async () => {
  try {
    const response = await fetch('http://localhost:8080/api/IT_Request_TableAdminController', {
      method: 'POST',
      credentials: 'include',
    });
...
const Status_update = async (index) => {
  try {
    await fetch('http://localhost:8080/api/IT_Request_StatusController', {
      method: 'POST',
      body: JSON.stringify({
...
const Approval_update = async (index) => {
  try {
    await fetch('http://localhost:8080/api/IT_Request_ApprovalController', {
      method: 'POST',
      body: JSON.stringify({
...
const saveAdminNote = async () => {
  try {
    await fetch('http://localhost:8080/api/IT_Request_AdminDetailController', {
      method: 'POST',
      body: JSON.stringify({
...
const saveManagerNote = async () => {
  try {
    await fetch('http://localhost:8080/api/IT_Request_ManagerDetailController', {
      method: 'POST',
      body: JSON.stringify({
