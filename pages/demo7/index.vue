<template>
    <div>
        ...
        <div class="text-center p-4 bg-sky-300">
            <h2 class="text-2xl font-semibold">Report_Request</h2>
        </div>
            ...
            <table class="table">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="(item, index) in filteredItems" :key="index">
                        ...
                        <td class="technician-column" style="text-align: left;  padding: 8px; ">{{ item.technician }}
                        </td>
  ...
  <div v-if="showFullDetailModal" class="modal-overlay">
    <div class="modal-content scrollable-content">
        ...
        <div>
            <ul>
                <li v-for="(accountItem, accountIndex) in selectedService.user_account.split(',')" :key="accountIndex">
                    <template v-if="accountItem">· {{ accountItem }}</template>
        ...
        <ul>
            <li v-for="(hardwareItem, hardwareIndex) in selectedService.hardware.split(',')" :key="hardwareIndex">
                <template v-if="hardwareItem">· {{ hardwareItem }}</template>
                <template v-else>·</template>
            </li>
        </ul>
        ...
        <ul>
            <li v-for="(softwareItem, softwareIndex) in selectedService.software_license.split(',')" :key="softwareIndex">
                <template v-if="softwareItem">· {{ softwareItem }}</template>
                <template v-else>·</template>
            </li>
        </ul>
        ...
        <ul>
            <li v-for="(erpItem, erpIndex) in selectedService.erp_mrp.split(',')" :key="erpIndex">
                <template v-if="erpItem">· {{ erpItem }}</template>
                <template v-else>·</template>
            </li>
        </ul>
        ...
        <ul>
            <li v-for="(fileItem, fileIndex) in selectedService.file_server.split(',')" :key="fileIndex">
                <template v-if="fileItem">· {{ fileItem }}</template>
                <template v-else>·</template>
            </li>
        </ul>
        ...
        <ul>
            <li v-for="(installationItem, installationIndex) in selectedService.installation.split(',')" :key="installationIndex">
                <template v-if="installationItem">· {{ installationItem }}</template>
                <template v-else>·</template>
            </li>
        </ul>
        ...
        <p style="text-align: left;">{{ selectedService.request_approval }}</p>
        <h3 class="sub-topic">หมายเหตุ</h3>
        <p style="text-align: left;">{{ selectedService.detail_manager }}</p> -->
